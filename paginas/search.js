...
module.exports = {
	exibir: function (requisicao, cliente, dados, database, template) {
		...
		db.query("select count(*) as total from (select id from transacoes where MATCH(speech) AGAINST('"+match+"' IN BOOLEAN MODE) LIMIT 1001) as tbl", function (err, total, fields) {
			...
			db.query("select * from transacoes where MATCH(speech) AGAINST('"+match+"' IN BOOLEAN MODE) limit "+pagina+",100", function (err, speech, fields) {
				...
				if(total>0){
					if(total>=1001){
					template[0]=template[0].replace(/{totalSpeech}/g, "more than 1000 clamspeech found, showing first 1000");
					}
					else{
